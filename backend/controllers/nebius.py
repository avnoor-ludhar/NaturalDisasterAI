  ...
  try:
    ...
    if(pc_chat_response and pc_image_response):
      completion = client.chat.completions.create(
        ...
        messages = [{
            "role": "system",
            "content": '''Your goal is to act as an AI chat bot to help people who are dealing with natural disasters and diseases. you may see posts about peoples concerns, help them navigate the situation with facts and useful information. you may also recieve 
            specific locations and posts. ENSURE TO ACT AS IF YOU ARE TALKING TO SOMEONE SO HAVE SOME BREVITY AT TIMES.'''
# posts about peoples concerns on natural disasters, and help users navigate the situation
          },
          *formatted_messages,
          {
            "role": "user",
...
def extract_key_item_from_prompt(prompt: str):
  completion = client.chat.completions.create(
    ...
    messages=[
      {
        "role": "system",
        "content": '''Your goal is to act as an assistant to help people who are dealing with natural disasters and diseases. you may see posts about peoples concerns, help them navigate the situation with facts and useful information. you may also recieve 
            specific locations and posts. ENSURE TO ACT AS IF YOU ARE TALKING TO SOMEONE SO HAVE SOME BREVITY AT TIMES. Help users navigate the warnings and risks and tell them the appropriate way to react to the situation.
        '''
      },
      {
        "role": "user",
...
class PDF(FPDF):
    def write_with_formatting(self, text):
        """method to handle basic markdown formatting"""
        lines = text.split("\n")
        for line in lines:
            if line.startswith("###"):
...
def generate_report(user_id: int):
  #check user id
    ...
    try:
        ...
        item_list = []
        for row in rows:
            if row['items']:  # Ensure items is not empty
                item_list.extend(row['items'].split(","))

        #duplicates and the list is sorted
        unique_items = sorted(set(item.strip() for item in item_list if item.strip()))

        # reports are generated based on llama
        ...
        if conn:
            conn.close()
##==================================================================================================
## REMINDER TO DELETE LATER!!!!
def generate_report_text(items: list) -> str:
    """Generate neatly-formatted report using Llama3.3."""
    prompt = f"""
